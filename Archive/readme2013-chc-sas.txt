  ...
  There are two ways to read the 2013 NAMCS Community Health Center (CHC) public use data file using SAS:

  Option 1 - Use the zipped file namcs2013_chc.zip in the SAS folder on 
  the FTP server to open a complete SAS dataset of the 2013 NAMCS CHC public use file.

  The steps for this option are as follows:

  1) Create a new folder on your local workstation, for example, C:\MyFiles\CHC2013

  2) Download to the new folder the file namcs2013_chc_sas.zip from the FTP server: 
     ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/dataset_documentation/namcs/sas

  3) namcs2013_chc_sas.zip is a compressed file which you must unzip prior to use. 
     ...
     called namcs2013_chc_sas.sas7bdat. This is the SAS dataset. You can then move the 
  ...
  4) To use the SAS dataset, the following code provides an example for a file saved to C:\MyFiles\CHC2013:

     libname out1 'C:\MyFiles\CHC2013';        /*point to file location on your workstation*/
     data test13; set out1.namcs2013_chc_sas;  /*create a temporary working file copied from the unzipped file*/
     proc surveyfreq data=test13;
  ...
  2) Download to the new folder the 2013 NAMCS CHC dataset (namcs2013_chc.zip) from the FTP server: 
     ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/datasets/namcs

  3) namcs2013_chc.zip is a compressed file which must be unzipped prior to use.
     In order to do this, double click on the file name in your directory screen; 
     an option to unzip the file should appear. Specify the desired location to unzip
     the file to and click on the Unzip option.  A new unzipped file should then appear,  
     called namcs2013_chc with no extension. This is the NAMCS CHC ASCII dataset. You can then 
     move the file to your preferred location. 

  4) Download to the new folder the chc13inp.txt, chc13lab.txt and chc13for.txt files from the FTP server:
     ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/dataset_documentation/namcs/sas

  5) Sample SAS code is shown below; other examples can be found here: 
     https://www.cdc.gov/nchs/ahcd/ahcd_presentations.htm

     filename chc13pub "C:\MyFiles\CHC2013\namcs2013_chc";         /*unzipped ASCII data set*/
     filename chc13for "C:\MyFiles\CHC2013\chc13for.txt";          /*SAS format statement*/
     filename chc13inp "C:\MyFiles\CHC2013\chc13inp.txt";          /*SAS input statement*/
     filename chc13lab "C:\MyFiles\CHC2013\chc13lab.txt";          /*SAS label statement*/

     %inc chc13for;  /*reads in the formats*/

     data test13; 
     infile chc13pub missover lrecl=9999;
     %inc chc13inp;  /*reads in the input statement*/
     %inc chc13lab;  /*reads in the labels*/
     run;
